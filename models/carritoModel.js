...
const getAll = async(id_user) => {
    const query = "SELECT c.id_producto, c.id, p.nombre, p.precio, p.descripcion, p.stock FROM ?? AS c JOIN ?? AS p ON c.id_producto = p.id WHERE c.id_usuario = ? and c.eliminado = 0";
...
module.exports = { getAll, insertCarrito, deleteCarrito, deleteProducto };